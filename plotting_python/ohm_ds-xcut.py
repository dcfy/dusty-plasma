...
def plotOhmY(ts, ix=None, iy=None, species=['e','p']):
  ...
  fig, ax = plt.subplots()
  #
  Ez = f.getField('Ez', ix=ix)*iEnorm
  #print 'Ez=', Ez, len(Ez)
  ax.plot(y, Ez, label='$ E_z $',color='k',lw=2,alpha=0.8)
  #
  Bx = f.getField('Bx', ix=ix)
  By = f.getField('By', ix=ix)
  #
  for s in species:
    # -VxB term, i.e., convection electric field
    vx_s = f.getField('vx_'+s, ix=ix)*iEnorm
    vy_s = f.getField('vy_'+s, ix=ix)*iEnorm
    _v_sxB = -vx_s*By + vy_s*Bx
    ax.plot(y, _v_sxB, label='$ -(\\mathbf{v}_%s \\times \\mathbf{B} )|_z$'%(s),lw=2,alpha=0.8)
  ...
  ax.set_xlabel('$y/d_{i0}$', fontsize='x-large')
  ax.set_ylabel('$E/B_0v_{A0}$', fontsize='x-large')
  ax.set_xlim([-20, 20])
  ax.set_ylim([-0.4, 0.1])
  fig.savefig('ohm_Ez_xcut_%02d_x%g.png'%(ts,x_cut), bbox_inches='tight')
  #plt.show()
  plt.close(fig)
  f.close()
