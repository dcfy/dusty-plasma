...
di0 = lightSpeed/wpi0
do0 = di0*math.sqrt(mo/mi)
de0 = di0*math.sqrt(me/mi)

-- perturbation magnetic flux function
psi0 = pert0 * di0 * B0
-- initial current sheet thickness
l = 0.9128709291752769*di0

-- box size
Lx = 100.0*di0 -- 25.6*di0
Ly = 50.0*di0  -- 12.8*di0
--Lx = 25.6*di0
--Ly = 12.8*di0
-- grid size
--Nx =  20
...
init = function(x,y,z)
   ...
   local n_i = n0 * icosh2y + nb
   local n_o = n0 * no_frac
   local n_e = n_i + n_o
   local rho_e = n_e*me
   local rho_i = n_i*mi
   local rho_o = n_o*mo
   ...
   local p_e = n_e * Te0
   local pzz_e = p_e + rhovz_e*rhovz_e/rho_e*n_e/(n0*icosh2y)
   local p_i = n_i * Ti0
   local pzz_i = p_i + rhovz_i*rhovz_i/rho_i*n_i/(n0*icosh2y)
   local p_o = n_o * To0 
   local pzz_o = p_o + rhovz_o*rhovz_o/rho_o
   --local u_o = 0.0 -- p_o / (gasGamma-1) + 0.5*rhovz_o^2/rho_o
   local vx_e = rhovx_e/rho_e
   local vy_e = rhovy_e/rho_e
   local vz_e = rhovz_e/rho_e
   
   local Ex = 0.0 -- -vy_e*Bz + vz_e*By
   local Ey = 0.0 -- -vz_e*Bx + vx_e*Bz
   local Ez = 0.0 -- -vx_e*By + vy_e*Bx

   return
      rho_e, rhovx_e, rhovy_e, rhovz_e, p_e, 0.0, 0.0, p_e, 0.0, pzz_e, 
      rho_i, rhovx_i, rhovy_i, rhovz_i, p_i, 0.0, 0.0, p_i, 0.0, pzz_i,
      rho_o, rhovx_o, rhovy_o, rhovz_o, p_o, 0.0, 0.0, p_o, 0.0, pzz_o,
      Ex, Ey, Ez, Bx, By, Bz, 0.0, 0.0
end
...
bcOxyCopy = BoundaryCondition.Copy { components = {20} }
bcOxyWall = BoundaryCondition.ZeroNormal { components = {21, 22, 23} }
bcOxyPrCopyY = BoundaryCondition.Copy { components = {24, 26, 27, 29} }
bcOxyPrFlipY = BoundaryCondition.Copy { components = {25, 28}, fact = {-1, -1} }

bcElcFld = BoundaryCondition.ZeroTangent { components = {30, 31, 32} }
bcMgnFld = BoundaryCondition.ZeroNormal { components = {33, 34, 35} }
bcPot = BoundaryCondition.Copy { components = {36, 37} }
...
function createBc(myDir, myEdge)
   local bc = Updater.Bc2D {
      ...
      boundaryConditions = {
         ...
         bcOxyCopy, bcOxyWall, bcOxyPrCopyY, bcOxyPrFlipY,
...
getFields = function(myQ)
   return myQ:alias(0,10), myQ:alias(10,20), myQ:alias(20,30), myQ:alias(30,38)
end
...
fluidEqn = HyperEquation.TenMoment { gasGamma = gasGamma, }
-- posistivity-conserving equation using Lax flux
--fluidEqnLax = HyperEquation.Euler { gasGamma = gasGamma, numericalFlux = "lax" }
fluidEqnLax = HyperEquation.TenMoment { gasGamma = gasGamma, numericalFlux = "lax" }
...
oxyCollisionlessSrcSlvr = Updater.TenMomentLocalCollisionlessHeatFlux2D {
    onGrid = grid,
    averageWaveNumber = ko0,
}
...
function updateSource(elcIn, ionIn, oxyIn, emfIn, tCurr, tEnd)
   ...
   --oxygen?
   oxyCollisionlessSrcSlvr:setOut( {oxyIn} )
   oxyCollisionlessSrcSlvr:setCurrTime(tCurr)
   oxyCollisionlessSrcSlvr:advance(tEnd)
